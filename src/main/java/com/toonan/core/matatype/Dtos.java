package com.toonan.core.matatype;
...
public class Dtos {
	 ...
	 * @return
	 */
	public static Dto newLsDto(HttpServletRequest request) {
		Dto newDto=new HashDto(request);
		String currentPage=newDto.getString("page");
		String pageSize=newDto.getString("limit");
		if(WebplusUtil.isNotEmpty(currentPage)){
			newDto.put("currentPage", currentPage);
			newDto.remove("page");
		}
		if(WebplusUtil.isNotEmpty(pageSize)){
			newDto.put("pageSize", pageSize);
			newDto.remove("limit");
		}
		return newDto;
	}
