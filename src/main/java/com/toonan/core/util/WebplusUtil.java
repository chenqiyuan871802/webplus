...
public class WebplusUtil {
	 ...
	 * @return
	 */
	public static String getWeekByCurDate() {
		SimpleDateFormat sdfInput = new SimpleDateFormat("yyyy-MM-dd");
		Calendar calendar = Calendar.getInstance();
		Date date = new Date();
		calendar.setTime(date);
	...
	/**
	 * 根据当前时间+随机数  获取指定长度的数字字符串（长度设定范围 14到50位）
	 * @param length
	 * @return
	 */
	public static String getRandomTimerId(int length){
		Date now=new Date();
		String res=now.getTime()+"";
		if(length<=14){
			length=14;
		}
		if(length>=50){
		...
		res=res+getRandomNumber(length-res.length());
		return res;
	}
	/**
	 * 获取指定长度的数字字符串
	...
	public static String getRandomNumber(int length){
		...
		StringBuffer res=new StringBuffer();
		while(length-->0){
			res.append(ran3.nextInt(10));//产生0-9的整数
		}
		return res.toString();
	}
	/**
	 ...
	 * 简要说明：转换原来key存储
	 * 编写者：陈骑元（chenqiyuan@toonan.com）
	 * 创建时间： 2020年12月9日 下午5:43:55 
	 * @param 说明
	 * @return 说明
	 */
	public static Dto convertKey(Dto dataDto,String srcKeys,String destKeys) {
		Dto     dataDtoNew=Dtos.newDto();
		if(WebplusUtil.isNotEmpty(dataDto)) {
			List<String> srcKeyList=WebplusFormater.separatStringToList(srcKeys);
			List<String> destList=WebplusFormater.separatStringToList(destKeys);
			int destSize=destList.size();
			
			for(int i=0;i<srcKeyList.size();i++) {
				String key=srcKeyList.get(i);
				if(dataDto.containsKey(key)) {
					if(i<destSize) {
						String destKey=destList.get(i);
						dataDtoNew.put(destKey, dataDto.get(key));
					}else {
						dataDtoNew.put(key, dataDto.get(key));
					}
				}
			}
		}
	...
	public static String getClientIpAddr(HttpServletRequest request) {
		...
		if (WebplusUtil.isEmpty(ip)) {
			ip = request.getHeader("X-Forward-For");
		}
		if (WebplusUtil.isEmpty(ip) || StringUtils.equalsIgnoreCase("unknown", ip)) {
	 ...
	 * 简要说明：根据索引获取逗号分隔的字符串
	 * 编写者：陈骑元（chenqiyuan@toonan.com）
	 * 创建时间： 2021年1月13日 下午3:45:10 
	 * @param 说明
	 * @return 说明
	 */
	public static String getListByIndex(int index,String str) {
		List<String> strList=WebplusFormater.separatStringToList(str);
		
		return  getListByIndex(index,strList);
	}
	/**
	 * 
	 * 简要说明：根据索引获取逗号分隔的字符串
	 ...
	 * @return 说明
	 */
	public static String getListByIndex(int index,List<String> strList) {
	 ...
	 * 简要说明：获取周一和周日日期
	 * 编写者：陈骑元（chenqiyuan@toonan.com）
	 * 创建时间： 2021年1月18日 上午10:20:50 
	 * @param 说明
	 * @return 说明
	 */
    public static Dto  getWeekFirstAndEndDate(String curDate) {
       
		try {
			
			     if(WebplusUtil.isEmpty(curDate)) {
			    	 curDate=WebplusUtil.getCurrentDate();
			     }
			     Dto  weekDto = Dtos.newDto();
		        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

		        Calendar cal = Calendar.getInstance();
		        Date  time = sdf.parse(curDate);
		        cal.setTime(time); 
		        ...
		        weekDto.put("firstWeekDate", weekBegin);
		        weekDto.put("endWeekDate", weekEnd);
		        return weekDto;
		} catch (ParseException e) {
