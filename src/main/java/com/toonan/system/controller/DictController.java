...
public class DictController extends BaseController {
     ...
     */
    @RequiresPermissions("system:dict:add")
    @PostMapping("batchSaveDict")
    @ResponseBody
    public R batchSaveDict(Dict dict,String firstDictCode) {
    	String userId=this.getUserId();
    	List<Dict> dictList=Lists.newArrayList();
    	int len=firstDictCode.length();
    	int firstDictInt=Integer.parseInt(firstDictCode);
    	String dictValue=dict.getDictValue();
    	Date now=WebplusUtil.getDateTime();
    	List<String> dictValueList=WebplusFormater.separatStringToList(dictValue);
    	for(int i=0;i<dictValueList.size();i++) {
    		Dict dictNew=new Dict();
    		int dictCodeInt=firstDictInt+i;
    		String dictCode=StringUtils.leftPad(dictCodeInt+"", len, "0");
    		dictNew.setDictCode(dictCode);
    		dictNew.setDictValue(dictValueList.get(i));
    		dictNew.setDictIndexId(dict.getDictIndexId());
    		dictNew.setSortNo(i+1);
    		dictNew.setStatus(WebplusCons.ENABLED_YES);
    		dictNew.setEditMode(WebplusCons.EDITMODE_EDIT);
    		dictNew.setCreateBy(userId);
        	dictNew.setUpdateBy(userId);
        	dictNew.setCreateTime(now);
        	dictNew.setUpdateTime(now);
        	dictList.add(dictNew);
    	}
        
    	if(WebplusUtil.isNotEmpty(dictList)) {
    		boolean result = dictService.insertBatch(dictList);
        	if (result) {
        		SystemCache.cacheDict(dict.getDictKey()); //刷新字典
        		return R.ok();
        	}
	 ...
	 * @return 说明
	 */
    @RequiresPermissions("system:dict:refreshCache")
